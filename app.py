...
from src.api_models import ChatRequest, ChatResponse, UploadResponse
...
app = FastAPI(
    title="TUNAJENGA — Self-RAG Copilot",
    version="2.0.0",
    description="Self-RAG copilot for company policies, services, and pricing.",
)
...
@app.get("/api/health")
def health():
    return {"status": "ok", "service": "tunajenga-self-rag"}
...
async def upload(file: UploadFile = File(...)):
    ...
    if suffix not in SUPPORTED:
        raise HTTPException(
            status_code=400, detail="Supported: PDF, TXT, MD, DOCX"
        )
